...
from gerenciamento_laboratorial.exame.api.serializers import ExameSerializer
from gerenciamento_laboratorial.exame.api.permissions import IsOwnerExameOrIsStaff
...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        if not request.user.is_staff:
            exames = self.queryset.filter(paciente=request.user.paciente, status=AGENDADO_STATUS)
        else:
            exames = self.queryset
    ...
    def destroy(self, request, pk=None):
        if not request.user.is_staff:
            ...
            exame.save()
            return Response(data={'detail': 'Solicitação de cancelamento realizada'}, status=status.HTTP_200_OK)
        return super().destroy(request, pk)

    @action(detail=False, methods=['get'], permission_classes=[IsAdminUser])
    def list_agendamentos(self, request):
        exames = self.queryset.filter(status=AGENDAMENTO_STATUS)
        serializer = self.get_serializer(exames, many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['get'], permission_classes=[IsAdminUser])
    def list_cancelamentos(self, request):
        exames = self.queryset.filter(status=CANCELAMENTO_STATUS)
        serializer = self.get_serializer(exames, many=True)
        return Response(serializer.data)
